...
class EfiPayIntegration
{
	...
	public function max_installments($total)
	{
		...
		$options = EfiPayIntegration::get_gn_api_credentials();
		...
		try {
			...
			$installments     = array();
			$installments     = $api->getInstallments($params, array());
			$max_installments = end($installments['data']['installments'])['installment'] . "x de " . EfiPayIntegration::formatCurrencyBRL(end($installments['data']['installments'])['value']);
	...
	public function get_installments($total, $brand)
	{
		...
		try {
			...
			return EfiPayIntegration::result_api($installments, true);
		} catch (EfiException $e) {
			$errorResponse = array(
				"code"    => $e->getCode(),
				"error"   => $e->error,
			...
			return EfiPayIntegration::result_api($errorResponse, false);
		} catch (Exception $e) {
			$errorResponse = array(
				"code"    => 0,
	...
	public function create_charge($order_id, $items, $shipping, $notification_url)
	{
		...
		try {
			...
			return EfiPayIntegration::result_api($charge, true);
		} catch (EfiException $e) {
			$errorResponse = array(
				"code"    => $e->getCode(),
				"error"   => $e->error,
	...
	public function pay_pix($options, $body)
	{
		...
		$clients =  EfiPayIntegration::get_gn_api_credentialsPix($options);
		$response = false;
		try {
			$api = new EfiPay($clients);
			$data = $api->pixCreateImmediateCharge([], $body);
			$response = true;
			return EfiPayIntegration::result_api($data, true);
		} catch (EfiException $e) {
			$data = array(
				"code" => $e->getCode(),
				"error" => $e->error,
			...
			return EfiPayIntegration::result_api($data, false);
		} catch (Exception $e) {
			$data = array("message" => $e->getMessage());
			return EfiPayIntegration::result_api($data, false);
		}

		return EfiPayIntegration::result_api($data, $response);
	}
	...
	public function update_meta($charge_id, $reference, $notification_url)
	{
		...
		try {
			...
			return EfiPayIntegration::result_api($subscription, true);
		} catch (EfiException $e) {
			$errorResponse = array(
				"code" => $e->code,
				"error" => $e->error,
	...
	public function notificationCheck($notificationToken)
	{
		...
		try {
			...
			return EfiPayIntegration::result_api($notification, true);
		} catch (EfiException $e) {
			$errorResponse = array(
	...
	public function getOpenFinanceParticipants($options)
	{
	...
	public function pay_openfinance($options, $body)
	{

		$clients =  EfiPayIntegration::get_gn_api_credentialsPix($options);
		$clients["headers"] = [
			"x-idempotency-key" => hash('sha256', time())
		];
		$response = false;
		try {
			$api = new EfiPay($clients);
	...
	public function config_app($options, $body)
	{
		$response = false;
		try {
			$api = new EfiPay($options);
